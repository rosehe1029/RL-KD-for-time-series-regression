...
def process_data(data_dir, data_identifier,winSize=30):
    ...
    if data_identifier == 'FD002' or data_identifier == 'FD004':
        ...
        Train_Norm = pd.DataFrame(columns = train_sensor.columns[3:],dtype=object)
        Test_Norm = pd.DataFrame(columns = test_sensor.columns[3:],dtype=object)
...
#         test_01_nor = np.delete(test_01_nor, [5,6,9,10,11,12,14,16,17,20,22,23], axis=1)
        ...
        max_RUL = 130.0 #125 To be consistent I made both 130
...
def save_dic_data(data_dir, saved_dir, window_size):
    ...
    for data_identifier in ['FD001','FD002','FD003','FD004']:
        ...
        cmapps_train_valid_test[data_identifier]={}
        cmapps_train_valid_test[data_identifier]['train_data']=data_proc[0]
        cmapps_train_valid_test[data_identifier]['valid_data']=data_proc[1]
        cmapps_train_valid_test[data_identifier]['test_data']=data_proc[2]
        cmapps_train_valid_test[data_identifier]['train_labels']=data_proc[3]
        cmapps_train_valid_test[data_identifier]['valid_labels']=data_proc[4]
        cmapps_train_valid_test[data_identifier]['test_labels']=data_proc[5]
    torch.save(cmapps_train_valid_test, saved_dir +'cmapss_train_valid_test_dic.pt')

def main():
    saved_dir = 'processed_data/'
    if not os.path.exists(saved_dir):
        os.makedirs(saved_dir)
    data_dir = 'data/'
    window_size =30
    save_dic_data(data_dir,saved_dir, window_size)
